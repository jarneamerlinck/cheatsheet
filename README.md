#  Table of Content
## [Protocols](Protocols/Protocol_overview.md)
### [I²C](Protocols/I%C2%B2C.md)
- [Information](Protocols/I%C2%B2C.md#Information)
- [Working](Protocols/I%C2%B2C.md#Working)
- [Setup](Protocols/I%C2%B2C.md#Setup)

### [UART](Protocols/UART.md#protocol-uart)
- [Information](Protocols/UART.md#Information)
- [Working](Protocols/UART.md#Working)
- [Setup](Protocols/UART.md#Setup)

### [SDI12](Protocols/SDI12.md#protocol-sdi12)
- [Information](Protocols/SDI12.md#Information)
- [Working](Protocols/SDI12.md#Working)
- [Commands](Protocols/SDI12.md#Commands)

### [SPI](Protocols/SPI.md#protocol-spi)
- [Information](Protocols/SPI.md#Information)
- [Working](Protocols/SPI.md#Working)
- [Commands](Protocols/SPI.md#Commands)

## [Infrastructure overview](infrastructure/infrastructure_overview.md)
### [Kubernetes](infrastructure/kubernetes.md#kubernetes)

#### [Kubernetes yaml examples](infrastructure/kubernetes_yaml.md#kubernetes-yaml)

### [Docker](infrastructure/docker_cheatsheet.md#docker_)

### [Keys-and-certificates](infrastructure/keys.md#keys-and-certificates)
- [ssh](infrastructure/keys.md#ssh-keys)
- [ssl](infrastructure/keys.md#ssl-certificates)
- [gpg](infrastructure/keys.md#gpg-keys)

### [Conda](infrastructure/Conda.md#conda)
- [Quick guide](infrastructure/Conda.md#quick-guide)
- [Channels and packages](infrastructure/Conda.md#channels-and-packages)
- [Working with conda environments](infrastructure/Conda.md#working-with-conda-environments)
- [Environment management](infrastructure/Conda.md#environment-management)
- [Exporting environments](infrastructure/Conda.md#exporting-environments)
- [Importing environments](infrastructure/Conda.md#importing-environments)

## Programming languages
### [Python](Programming_languages/Python.md#python)
- [Information](Programming_languages/Python.md#information)
- [Data Type](Programming_languages/Python.md#data-type)
- [Default Commands](Programming_languages/Python.md#default-commands)
- [Libraries](Programming_languages/Python_libraries.md#python-libraries)

### [R](Programming_languages/R.md#r)
- [Information](Programming_languages/R.md#information)
- [Prediction Models](Programming_languages/R.md#prediction-models)

### [C](Programming_languages/C.md#c)
- [Data types](Programming_languages/C.md#data-types)
- [Pointers](Programming_languages/C.md#Pointers)
- [Boilerplates](Programming_languages/C.md#Boilerplates)

## [Linux overview](linux/linux_overview.md)
### [Bash](linux/bash_cheatsheet.md)
- [Networking](linux/bash_cheatsheet.md#networking)
- [Permissions](linux/bash_cheatsheet.md#file-permissions)
- [Scripts](linux/bash_cheatsheet.md#scripts)

### [Package managers](linux/package_managers.md)
- [nala](linux/package_managers.md#nala)
- [packman](linux/package_managers.md#packman)
- [dnf](linux/package_managers.md#dnf)
- [nix](linux/package_managers.md#nix)


### [Tmux](linux/tmux_cheatsheet.md)
### [Vim](linux/vim_cheatsheet.md)


### [Linux gaming](linux/linux_gaming.md)
### [Chezmoi](linux/chezmoi.md)

## Other
### [Markdown](Markdown_cheatsheet.md)
- [Basic syntax](Markdown_cheatsheet.md#basic-syntax)
- [Extended syntax](Markdown_cheatsheet.md#extended-syntax)


### [Git](git_cheatsheet.md)

- [Git](git_cheatsheet.md#git)
  - [Configs](git_cheatsheet.md#configs)
  - [Normal Commands](git_cheatsheet.md#normal-commands)
    - [Bisect](git_cheatsheet.md#bisect)
    - [Add Patch](git_cheatsheet.md#add-patch)
  - [Remotes](git_cheatsheet.md#remotes)
  - [Logs](git_cheatsheet.md#logs)
  - [Stach](git_cheatsheet.md#stach)
  - [Branches](git_cheatsheet.md#branches)
    - [Merge](git_cheatsheet.md#merge)
      - [Merge error](git_cheatsheet.md#merge-error)
      - [Merge Types](git_cheatsheet.md#merge-types)
      - [Merge strategy](git_cheatsheet.md#merge-strategy)
      - [Merge strategy options for Recursive strategy](git_cheatsheet.md#merge-strategy-options-for-recursive-strategy)
